...
BlitStage::BlitStage(const std::shared_ptr<Engine> &engine, const std::shared_ptr<VoxelRenderSettings> &settings,
                     const RenderPass &renderPass) : AVoxelRenderStage(engine, settings) {
    _offsetsBuffer = std::make_unique<Buffer>(engine, sizeof(BlitOffsets), vk::BufferUsageFlagBits::eUniformBuffer,
                                              VMA_MEMORY_USAGE_CPU_TO_GPU, "Blit Offsets Buffer");
    ...
    engine->recreationQueue->push(RecreationEventFlags::TARGET_RESIZE, [&]() {
        _pipeline->descriptorSet->initBuffer(1, _offsetsBuffer->buffer, _offsetsBuffer->size,
...
void BlitStage::record(const vk::CommandBuffer &cmd, uint32_t flightFrame, const RenderImage &image,
    ...
    _pipeline->descriptorSet->writeBuffer(1, flightFrame, _offsetsBuffer->buffer, sizeof(BlitOffsets),
                                          vk::DescriptorType::eUniformBuffer);
