...
void
DescriptorSet::initBuffer(uint32_t binding, vk::Buffer buffer, vk::DeviceSize size, vk::DescriptorType type) const {
    for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
        writeBuffer(binding, i, buffer, size, type);
}

void DescriptorSet::initImage(uint32_t binding, vk::ImageView imageView, vk::Sampler sampler,
...
DescriptorSet DescriptorSetBuilder::build(const std::string &name) {
    ...
    descriptorSet.pushDeletor([=](const std::shared_ptr<Engine> &delEngine) {
        delEngine->device.freeDescriptorSets(delEngine->descriptorPool,
                                             static_cast<uint32_t>(sceneDescriptorAllocResult.size()),
                                             sceneDescriptorAllocResult.data());
    });
